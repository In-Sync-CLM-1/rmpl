...
SELECT project_owner AS user_id,
    date_trunc('month'::text,
        CASE
            WHEN event_dates IS NOT NULL AND jsonb_array_length(event_dates) > 0 THEN ((event_dates -> 0) ->> 'date'::text)::date
   ...
   FROM projects
  WHERE status = 'invoiced' AND (final_afactor IS NOT NULL OR expected_afactor IS NOT NULL)
  GROUP BY project_owner, (date_trunc('month'::text,
        CASE
            WHEN event_dates IS NOT NULL AND jsonb_array_length(event_dates) > 0 THEN ((event_dates -> 0) ->> 'date'::text)::date
            ELSE updated_at::date
