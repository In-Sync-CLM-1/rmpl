-- Drop the incorrectly created function with 5 parameters
DROP FUNCTION IF EXISTS public.process_bulk_import_batch(uuid, integer, jsonb, text, uuid);

-- Update the existing function (with 4 parameters) to include assigned_to handling
...
BEGIN
  -- Process staging records for this import
      ...
      IF p_table_name = 'demandcom' THEN
        ...
        IF v_assigned_to_email IS NOT NULL AND v_assigned_to_email != '' THEN
          ...
          IF v_assigned_to_id IS NULL THEN
            RAISE WARNING 'User not found for email: %', v_assigned_to_email;
          END IF;
        END IF;
        ...
        ELSE
          ...
          INSERT INTO demandcom (
            name, salutation, company_name, designation, deppt,
            mobile_numb, official, personal_email_id, generic_email_id, mobile2,
            linkedin, company_linkedin_url, associated_member_linkedin, website,
            address, city, state, country, pincode, zone, location, head_office_location,
            industry_type, sub_industry, emp_size, turnover, turnover_link, tier,
            erp_name, erp_vendor, job_level_updated, source, source_1,
            activity_name, remarks, extra, extra_1, extra_2,
      ...
      ELSIF p_table_name = 'master' THEN
        ...
        ELSE
          ...
          INSERT INTO master (
            name, salutation, company_name, designation, deppt,
            mobile_numb, official, personal_email_id, generic_email_id, mobile2,
            linkedin, website, address, city, state, country, pincode, zone, location,
            industry_type, sub_industry, emp_size, turnover, tier, source, remarks,
