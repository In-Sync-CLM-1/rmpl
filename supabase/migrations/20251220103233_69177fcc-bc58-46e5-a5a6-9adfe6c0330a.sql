-- Drop and recreate the csbd_actuals view to use event_dates instead of updated_at
DROP VIEW IF EXISTS public.csbd_actuals;
...
SELECT 
    ...
    date_trunc('month', 
        CASE 
            WHEN event_dates IS NOT NULL AND jsonb_array_length(event_dates) > 0 
            THEN (event_dates->0->>'date')::date
