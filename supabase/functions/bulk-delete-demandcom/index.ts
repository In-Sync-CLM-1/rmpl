...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!is_continuation) {
      console.log(`User ${user.email} (${user.id}) starting delete for ${recordIds.length} records`);
    } else {
    ...
    const { data, error } = await adminSupabase.rpc('bulk_delete_demandcom_batch', {
    ...
    if (error) {
      console.error('Error calling bulk_delete_demandcom_batch:', error);
    ...
    if (hasMore) {
      ...
      const selfUrl = `${supabaseUrl}/functions/v1/bulk-delete-demandcom`;
    ...
    // All done
    console.log(`Completed bulk delete. Total deleted: ${totalDeleted}`);
