...
import { useProjectLiveCom } from "@/hooks/useProjectLiveCom";
import { ProjectLiveComDialog } from "./ProjectLiveComDialog";
...
interface ProjectLiveComManagerProps {
  projectId: string;
}

export function ProjectLiveComManager({ projectId }: ProjectLiveComManagerProps) {
  const {
    checklistItems,
    isLoading,
    initializeChecklist,
    isInitializing,
    updateChecklistItem,
  } = useProjectLiveCom(projectId);

  const [selectedItem, setSelectedItem] = useState<LiveComChecklistItem | null>(
    null
  );
  ...
  const groupedItems = checklistItems.reduce(
    ...
    {} as Record<string, LiveComChecklistItem[]>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-muted/50 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold">LiveCom Checklist Progress</h3>
